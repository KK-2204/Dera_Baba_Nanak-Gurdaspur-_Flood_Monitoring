...
1. **`Dera_Baba_Nanak(Gurdaspur)_Flood_Detection.js`** — Google Earth Engine script covering the full pipeline: AOI setup, 3-date NDVI (pre-flood / peak-flood / recovery), NDWI, vegetation loss change detection, and two independent SAR (Sentinel-1) flood detection methods with a full statistical comparison.
